...
const ContestCreation = () => {
  ...
  const createContest = async (e) => {
    ...
    try {
      const response = await axios.post(`${Baseurl}/post/createcontest`, contestPayload, {
        withCredentials: true
      });
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-10 border border-gray-100">
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-bold text-indigo-800">Create a New Contest</h2>
          <p className="text-gray-600 text-sm">Fill in the details to set up your programming challenge</p>
        </div>
        ...
        <form onSubmit={createContest} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="block text-gray-700 text-sm font-semibold" htmlFor="description">
                Description
              </label>
              <textarea
                className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            ...
            {/* Start Date and Time */}
            <div className="space-y-2">
              <label className="block text-gray-700 text-sm font-semibold" htmlFor="startDate">
                Start Date
              </label>
              <input
                className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            ...
            <div className="space-y-2">
              <label className="block text-gray-700 text-sm font-semibold" htmlFor="startTime">
                Start Time
              </label>
              <input
                className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            ...
            {/* End Date and Time */}
            <div className="space-y-2">
              <label className="block text-gray-700 text-sm font-semibold" htmlFor="endDate">
                End Date
              </label>
              <input
                className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            ...
            <div className="space-y-2">
              <label className="block text-gray-700 text-sm font-semibold" htmlFor="endTime">
                End Time
              </label>
              <input
                className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-10">
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 px-6 py-4 border-b border-gray-200">
          ...
          <p className="text-gray-600 text-sm">Manage and add questions to your created contests</p>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {contests.map(contest => (
                ...
                >
                  <div className={`h-2 ${contest.isPublic ? 'bg-green-600' : 'bg-gray-600'}`}></div>
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-3">
                      <h3 className="font-bold text-xl text-gray-800 line-clamp-1">{contest.title}</h3>
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        contest.isPublic ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                      }`}>
                        {contest.isPublic ? 'Public' : 'Private'}
                      </span>
                    </div>
                    
                    <p className="text-gray-600 mb-4 line-clamp-2 h-12">{contest.description}</p>
                    ...
                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                      <div className="flex items-center text-sm">
                        ...
                        <span className="font-medium">{contest.QuestionSet?.length || 0} Questions</span>
                      </div>
                    </div>
                    
                    <div className="flex mt-4 space-x-2">
                      <button
                        onClick={() => toggleContestVisibility(contest.id || contest._id, contest.isPublic)}
                        className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-colors shadow-md ${
                          contest.isPublic 
                            ? 'bg-gray-200 hover:bg-gray-300 text-gray-800' 
                            : 'bg-green-500 hover:bg-green-600 text-white'
                        }`}
                      >
                        {contest.isPublic ? 'Make Private' : 'Make Public'}
                      </button>
            ...
            <div className="text-center py-16 bg-gray-50 rounded-xl">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">No contests created yet</h3>
              <p className="mt-2 text-gray-500 max-w-md mx-auto">
                Get started by creating your first contest using the form above. Once created, you can add questions to your contest.
              </p>
            </div>
          )}
        </div>
      ...
      {/* Question Form Section - Only shown when a contest is selected */}
      {selectedContestId && (
        <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-10 border border-gray-100 animate-fadeIn">
          ...
          <div className="p-6">
            {/* Pass the contestId prop to the imported QuestionForm component */}
            <QuestionForm
              contestId={selectedContestId} 
              onComplete={closeQuestionForm} 
            />
          </div>
        </div>
      )}
